class Solution:
    def around_cases(self, heights: list[list[int]], pos: tuple[int, int]) -> list[tuple[int, int]]:
        ...
        for dl, dc in directions:
            ...
            if (
                0 <= nouvelle_ligne < len(heights)
                and 0 <= nouvelle_colonne < len(heights[0])
                and heights[nouvelle_ligne][nouvelle_colonne] >= valeur_actuelle
            ):
    ...
    def pacificAtlantic(self, heights: list[list[int]]) -> list[list[int]]:
        pacific_water = [(0, i) for i in range(len(heights[0]))]
        pacific_water += [(i, 0) for i in range(len(heights))]

        pacific_touched = self.getTouchedWater(heights, pacific_water)

        atlantic_water = [(len(heights) - 1, i) for i in range(len(heights[0]))]
        atlantic_water += [(i, len(heights[0]) - 1) for i in range(len(heights))]
